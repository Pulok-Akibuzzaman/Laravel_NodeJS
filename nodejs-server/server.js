...
app.get('/', (req, res) => {
    ...
    const webAppUrl = isLocal ? 'http://localhost:8000' : 'https://task-laravel-api.onrender.com';
    ...
    res.send(`
        <div style="font-family: system-ui, sans-serif; max-width: 600px; margin: 4rem auto; padding: 2rem; background: #0f172a; color: #f8fafc; border-radius: 16px; border: 1px solid rgba(255,255,255,0.1); box-shadow: 0 10px 30px rgba(0,0,0,0.5);">
            ...
            <p style="margin-top: 1.5rem;">👉 Open the Task Manager Web App at: <a href="${webAppUrl}" style="color: #38bdf8; font-weight: 600; text-decoration: underline;">${webAppUrl}</a></p>
        </div>
    `);
});
